...
class ImageExtractor:
    ...
    async def _extract_logos(self, soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
        ...
        svg_elements = soup.find_all('svg', class_=lambda x: x and 'logo' in x.lower() if x else False)
    ...
    async def _extract_favicons(self, soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
            ...
            for element in elements:
                ...
                if href:
                    favicons.append({
                        ...
                        "type": element.get('rel', ['favicon'])[0] if isinstance(element.get('rel'), list) else element.get('rel', 'favicon'),
    ...
    async def _extract_og_images(self, soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
        ...
        if twitter_image and twitter_image.get('content'):
            og_images.append({
                "url": self._make_absolute_url(twitter_image.get('content'), base_url),
                "type": "twitter_image",
    ...
    async def _extract_general_images(self, soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
